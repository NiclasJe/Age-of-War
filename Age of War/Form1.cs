...
namespace Age_of_War
{
  ...
  private System.Windows.Forms.Timer gameTimer;
    ...
    private UnitButton btnBasicTurret, btnAdvancedTurret, btnPremiumTurret;
        private UnitButton btnSellTurret;
     private UnitButton btnExpandBase;
   private UnitButton btnToggleSoldiers;
      private bool showingTurrets = false;
        private Type selectedTurretType = null;
...
InitializeComponent();
   ...
   this.FormBorderStyle = FormBorderStyle.Sizable;
         ...
         this.StartPosition = FormStartPosition.CenterScreen;
            ...
            mainMenu = new MainMenu(this.ClientSize.Width, this.ClientSize.Height);

            // Battlefield panel with double buffering
     ...
     Size = new Size(this.ClientSize.Width, this.ClientSize.Height),
      BackColor = Color.Transparent,
       ...
       battlefieldPanel.MouseClick += BattlefieldPanel_MouseClick;
            battlefieldPanel.MouseMove += BattlefieldPanel_MouseMove;
  ...
  gameTimer = new System.Windows.Forms.Timer { Interval = 33 }; // ~30 FPS
     ...
     gameTimer.Start();
      ...
      // Set all UI as invisible initially (main menu is shown first)
...
pausePanel.Controls.Add(pauseTitle);
...
Text = "? FORTSÄTT",
    ...
    ForeColor = Color.White,
         ...
         btnResume.FlatAppearance.BorderColor = Color.White;
         btnResume.FlatAppearance.BorderSize = 2;
         btnResume.Click += BtnResume_Click;

            btnExitPause = new Button
         {
     Text = "? AVSLUTA",
            Size = new Size(250, 60),
      ...
      FlatStyle = FlatStyle.Flat
            ...
            btnExitPause.FlatAppearance.BorderColor = Color.White;
        btnExitPause.FlatAppearance.BorderSize = 2;
            btnExitPause.Click += BtnExit_Click;

 pausePanel.Controls.Add(btnResume);
        pausePanel.Controls.Add(btnExitPause);
 ...
 btnToggleTurret = new UnitButton(typeof(BasicTurret), 0) { Location = new Point(350, 20) };
...
btnRanged.BringToFront();
 ...
 btnBasicTurret = new UnitButton(typeof(BasicTurret), 500) { Location = new Point(20, 20), Visible = false };
  ...
  btnPremiumTurret = new UnitButton(typeof(PremiumTurret), 1500) { Location = new Point(240, 20), Visible = false };
         btnSellTurret = new UnitButton(null, 0, true) { Location = new Point(350, 20), Visible = false };
   btnExpandBase = new UnitButton(null, 0, false, true) { Location = new Point(460, 20), Visible = false };
            btnToggleSoldiers = new UnitButton(typeof(Soldier), 0) { Location = new Point(570, 20), Visible = false };
...
btnToggleSoldiers.Click += (s, e) => ToggleToSoldiers();
  ...
  btnToggleSoldiers.BringToFront();
       ...
       btnUltimate.Visible = visible;
            btnSword.Visible = visible;
            btnRanged.Visible = visible;
...
game.UpdateDimensions(this.ClientSize.Width, this.ClientSize.Height);
 ...
 {
    int cost = unitType == typeof(Soldier) ? 50 : unitType == typeof(RangedUnit) ? 80 : 200;
...
if (game.PlayerTurrets.Count == 0)
 ...
 private void BattlefieldPanel_MouseMove(object sender, MouseEventArgs e)
     ...
     if (currentState == GameState.MainMenu)
            {
                mainMenu.CheckHover(e.Location);
    battlefieldPanel.Invalidate();
            }
 else if (currentState == GameState.GameOver)
    {
        endScreen.CheckHover(e.Location);
                battlefieldPanel.Invalidate();
    }

     if (previewTurret != null && currentState == GameState.Playing)
      {
 ...
 // Uppdatera ultimate-knappen
     ...
     // Kontrollera vinst/förlust
         if (game.PlayerBase.HP <= 0 || game.EnemyBase.HP <= 0)
            {
  bool playerWon = game.EnemyBase.HP <= 0;
                currentState = GameState.GameOver;
       endScreen = new EndScreen(playerWon, this.ClientSize.Width, this.ClientSize.Height);
      SetGameUIVisibility(false);
         pausePanel.Visible = false;
            }
    ...
    private void BattlefieldPanel_MouseClick(object sender, MouseEventArgs e)
{
     if (currentState == GameState.MainMenu)
  {
              if (mainMenu.IsPlayButtonClicked(e.Location))
        {
     StartGame();
     }
    else if (mainMenu.IsExitButtonClicked(e.Location))
           {
Application.Exit();
                }
                return;
            ...
            if (currentState == GameState.GameOver)
            {
             if (endScreen.IsPlayAgainButtonClicked(e.Location))
           {
      StartGame();
     }
      else if (endScreen.IsExitButtonClicked(e.Location))
     {
     Application.Exit();
 }
          return;
  ...
  for (int i = 0; i < game.TurretSlots.Count; i++)
       ...
       var slot = game.TurretSlots[i];
         ...
         {
     if (game.PlaceTurret(selectedTurretType, selectedTurretCost, i))
  {
            selectedTurretType = null;
      selectedTurretCost = 0;
    previewTurret = null;
 ...
 var g = e.Graphics;
            ...
            if (currentState == GameState.MainMenu)
  {
          mainMenu.Draw(g, this.ClientSize.Width, this.ClientSize.Height);
      return;
            }

            if (currentState == GameState.GameOver)
            {
 // Draw last game frame first
   DrawGameScene(g);
        // Draw end screen overlay
   endScreen.Draw(g, this.ClientSize.Width, this.ClientSize.Height);
         return;
            }

      // Draw normal game scene
       DrawGameScene(g);
 }

        private void DrawGameScene(Graphics g)
        {
            // (The rest of the drawing code will be in the next part of Form1.DrawHelpers.cs partial class)
 ...
 DrawHill(g, this.ClientSize.Width - 400, game.BattlefieldY - 170, 500, 170, Color.FromArgb(95, 115, 95));
    ...
    Color midHillColor = Color.FromArgb(80, 140, 60);
     ...
     DrawForest(g, 200, game.BattlefieldY - 60, 6);
            DrawHill(g, 400, game.BattlefieldY - 120, 400, 120, Color.FromArgb(75, 130, 55));
      DrawForest(g, 650, game.BattlefieldY - 65, 3);
 ...
 // Blodpartiklar
  ...
  popup.Draw(g);
   ...
   int baseX = game.PlayerBase.Hitbox.X;
          ...
          int towerLevel = i + 1;
 int towerX = baseX + 20;
            int towerY = baseY - (towerLevel * 50) - 10;
  int towerHeight = 50;
    int towerWidth = 60;
